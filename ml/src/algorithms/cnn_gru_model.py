...
class FallDetectorCNN_GRU(nn.Module):
    """Same CNN feature extractor as the baseline LSTM model, but with a GRU temporal head."""
    ...
    def __init__(self) -> None:
        ...
        self.head_fall = nn.Sequential(nn.Linear(128, 1), nn.Sigmoid())
...
def train_and_evaluate(epochs: int = N_EPOCHS, batch_size: int = BATCH_SIZE, lr: float = LR, save_metrics: bool = True) -> dict:
    ...
    X_train, X_val, X_test, labels_train, labels_val, labels_test, idx_test = train.split_data(X, labels)
    ...
    for epoch in range(1, epochs + 1):
        ...
        print(f"[GRU] epoch {epoch}/{epochs} train_loss={train_loss:.4f} val_loss={val_loss:.4f}")
    ...
    if save_metrics:
        output_path = RESULTS_DIR / "gru_metrics.json"
        with open(output_path, "w", encoding="utf-8") as handle:
            json.dump(metrics, handle, indent=2)
        print(f"[GRU] Metrics saved to {output_path}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Train and evaluate the CNN-GRU ablation model")
